...
"""
该执行文件主要用于训练基于Bert Further pretrain的预训练模型-model_2_bert_att_drop_further_pretrain,并生成测试集的预测概率文件
"""
...
patience = 5   # 早停轮数
file_name = 'model_2_bert_further_pretrain'  # 指定输出文件的名字
model_name_or_path = './pretrain_weight/bert_further_pretrain/'  # 预训练模型权重载入路径
...
for fold, (train_index, valid_index) in enumerate(skf.split(all_label, all_label)):
    ...
    model.load_state_dict(torch.load('./model_save/bert_cv_' + file_name + '_{}.bin'.format(fold)))
    model.eval()
    with torch.no_grad():
